    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                Upload Files for {{ $taxpayer->name }}
            </h2>


            <a href="{{ route('interviewer.taxpayer.taxpayeruploads', $taxpayer) }}"
               class="inline-flex items-center px-3 py-2 bg-gray-200 hover:bg-gray-300
                      dark:bg-gray-700 dark:hover:bg-gray-600 rounded
            ...
            <div class="p-6">
                ...
                >
                    ...
                    <p class="text-xs text-gray-500">
                        Accepted: PDF, images, DOC/XLS, CSV, TXT (≤ 10MB each)
                    </p>
                ...
                <div class="mt-6" x-show="queue.length">
                    <h3 class="text-sm font-semibold mb-2">
                        Queued Files (<span x-text="queue.length"></span>)
                    </h3>
                        ...
                        <template x-for="(f, idx) in queue" :key="idx">
                            <li class="flex items-center justify-between p-3 rounded
                                ...
                                <div class="truncate">
                                    ...
                                    <span class="text-xs text-gray-500" x-text="human(f.size)"></span>
                                </div>
                                <div class="w-1/3">
                                    <div class="h-2 bg-gray-200 dark:bg-gray-700 rounded">
                                        <div class="h-2 bg-green-600 rounded"
                                             :style="`width:${progress[f.name]||0}%`"></div>
                                    </div>
                                </div>
                            </li>
                        </template>
                        ...
                        @foreach($uploads as $u)
                            <div class="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                                <div class="p-3">
                                    ...
                                    <p class="text-xs text-gray-500">
                                        {{ strtoupper($u->mime ?? 'file') }}
                                        • {{ number_format((int)$u->size/1024,0) }} KB
                                    </p>
                                </div>

                                @php $url = asset('storage/'.$u->path); @endphp

                                @if(isset($u->mime) && str_starts_with($u->mime, 'image/'))
            ...
            return {
                ...
                uploadOne(file){
                    return new Promise(resolve => {
                        ...
                        xhr.open(
                            'POST',
                            '{{ route('interviewer.taxpayer.uploads.taxpayerstore', $taxpayer) }}'
                        );
